...
class  FM_RenderBlock {
   ...
   //funzione richiamata come callback da gutenberg
	public function FM_render_block_callback( $block_attributes, $content ) {
		...
		$this->parametri_blocco = $this->validate_block_parameters($block_attributes);
		...
		//controllo che la query sia corretta
		if ( $this->query_interna instanceof \WP_Query){
			...
			error_log('Errore: $this->query_interna non è un\'istanza valida di WP_Query');
	}		
	...
	// Funzione per validare i parametri del blocco
	private function validate_block_parameters($params) {
		$validated_params = [];

		// Validazione del parametro 'TipoQuery'
		if (isset($params['TipoQuery']) && in_array($params['TipoQuery'], ['wp-query', 'custom-query', 'no-query'])) {
			$validated_params['TipoQuery'] = $params['TipoQuery'];
		} else {
			$validated_params['TipoQuery'] = 'wp-query'; // Default
		}

		// Validazione del parametro 'postsToShow'
		if (isset($params['postsToShow']) && is_numeric($params['postsToShow']) && $params['postsToShow'] > 0) {
			$validated_params['postsToShow'] = (int) $params['postsToShow'];
		} else {
			$validated_params['postsToShow'] = get_option('posts_per_page'); // Default
		}

		// Validazione del parametro 'displayPagination'
		$validated_params['displayPagination'] = isset($params['displayPagination']) ? (bool) $params['displayPagination'] : false;

		// Validazione del parametro 'categories'
		if (isset($params['categories']) && is_array($params['categories'])) {
			$validated_params['categories'] = array_filter($params['categories'], function($category) {
				return isset($category['id']) && is_numeric($category['id']);
			});
		} else {
			$validated_params['categories'] = [];
		}
		...
		if (isset($params['order']) && in_array($params['order'], ['ASC', 'DESC'])) {
			...
			$validated_params['order'] = 'DESC'; // Default
		}

		// Validazione del parametro 'orderBy'
		if (isset($params['orderBy']) && is_string($params['orderBy'])) {
			$validated_params['orderBy'] = $params['orderBy'];
		} else {
			$validated_params['orderBy'] = 'date'; // Default
		}

		// Validazione del parametro 'displayPostContent'
		$validated_params['displayPostContent'] = isset($params['displayPostContent']) ? (bool) $params['displayPostContent'] : false;

		// Validazione del parametro 'displayFeaturedImage'
		$validated_params['displayFeaturedImage'] = isset($params['displayFeaturedImage']) ? (bool) $params['displayFeaturedImage'] : false;


		// Validazione del parametro 'Visualizzazione'
		if (isset($params['Visualizzazione']) && is_string($params['Visualizzazione']) && !empty($params['Visualizzazione'])) {
			$validated_params['Visualizzazione'] = $params['Visualizzazione'];
		} else {
			$validated_params['Visualizzazione'] = WP_PLUGIN_DIR . '/format-adv-query-ext-pattern/fm_advq_patterns/ViewDefault.json'; // Default
		}

		// Validazione del parametro 'queryTypeAdvanced'
		$validated_params['queryTypeAdvanced'] = isset($params['queryTypeAdvanced']) ? (bool) $params['queryTypeAdvanced'] : false;

		// Restituisco i parametri validati
	...
	//funzione per settare la lunghezza del riassunto
	 ...
	 private function FM_gestisci_visualizzazione_da_blocco(){
		 ...
		 if ($this->parametri_blocco['displayPostContent']) {
